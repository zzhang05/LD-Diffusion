...
def training_loop(
    ...
    stats_jsonl = None
    batch_mul_dict = {512: 1, 256: 2, 192: 8/3, 128: 4, 64: 16, 32: 32, 24: 128/3, 16: 64, 8: 128, 4: 256}
    if train_on_latents:
        p_list = np.array([(1-real_p), real_p])
        patch_list = np.array([img_resolution//(4/3), img_resolution])
        batch_mul_avg = np.sum(p_list * np.array([4/3, 1]))  # 2
    else:
        p_list = np.array([(1-real_p), real_p])
        patch_list = np.array([img_resolution//(4/3), img_resolution])
        batch_mul_avg = np.sum(np.array(p_list) * np.array([4/3, 1])) # 2
    while True:
        ...
        for round_idx in range(num_accumulation_rounds):
            with misc.ddp_sync(ddp, (round_idx == num_accumulation_rounds - 1)):
                ...
                batch_mul = int(batch_mul_dict[patch_size] // batch_mul_dict[img_resolution])
                images, labels = [], []
